...
/**
 * XenChat and WordPress users DAO.
 *
 * @author Kainex <contact@kainex.pl>
 */
class XenChatUsersDAO {
	/**
	* @var XenChatOptions
	*/
	private $options;
	...
	public function get($id) {
		...
		$table = XenChatInstaller::getUsersTable();
	...
	/**
	 * Converts raw object into XenChatUser object.
	 *
	...
	*/
	public function hasCurrentWpUserRight($rightName) {
		...
		if ($wpUser !== null) {
			...
			if ((is_array($wpUser->roles) && count(array_intersect($targetRoles, $wpUser->roles)) > 0) || current_user_can("xen_chat_{$rightName}")) {
				return true;
			}
	...
	*/
	...
	public function getWPUsers($limitToUsers = array()) {
		...
		if ($usersCacheTime > 0) {
			$transientKey = 'xen_chat_wp_users_cache';
