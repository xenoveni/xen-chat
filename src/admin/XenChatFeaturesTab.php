 ...
 */
class XenChatFeaturesTab extends XenChatAbstractTab {
	...
	public function customReactionsCallback() {
		...
		for ($i = 1; $i <= 7; $i++) {
			...
			$idInput = sprintf(
				...
				XenChatOptions::OPTIONS_NAME, $key, $i
			);
			$actionInput = sprintf(
				'<input type="text" name="%s[reactions_custom][%d][action]" disabled value="%s" maxlength="100" style="max-width: 100px;">%s',
				XenChatOptions::OPTIONS_NAME, $key, htmlspecialchars($custom[$key]['action']), $i === 1 ? '<p class="description">e.g. Like</p>' : ''
			);
			$activeInput = sprintf(
				'<input type="text" name="%s[reactions_custom][%d][active]" disabled value="%s" maxlength="100" style="max-width: 100px;">%s',
				XenChatOptions::OPTIONS_NAME, $key, htmlspecialchars($custom[$key]['active']), $i === 1 ? '<p class="description">e.g. I like it</p>' : ''
			);
			...
			$imageInput = sprintf(
				...
				htmlspecialchars($custom[$key]['image']), $i, XenChatOptions::OPTIONS_NAME, $key, $i, $imageTag, $i, $i, $i === 1 ? '<p class="description">max. 48x48</p>' : ''
			);
			...
			$imageSmInput = sprintf(
				...
				htmlspecialchars($custom[$key]['imageSm']), $i, XenChatOptions::OPTIONS_NAME, $key, $i, $imageSmTag, $i, $i, $i === 1 ? '<p class="description">max. 48x48</p>' : ''
			);
	...
	public function reactionsActionsCallback() {
		$url = admin_url("options-general.php?page=".XenChatSettings::MENU_SLUG."&wc_action=clearAllReactions");
