...
/**
 * Xen Chat admin channels settings tab class.
 *
 * @author Kainex <contact@kainex.pl>
 */
class XenChatChannelsTab extends XenChatAbstractTab {
    ...
    public function deleteChannelAction() {
        ...
        $this->addMessage(
            ...
            Note: You have to remove [xen-chat] shortcode as well. If you don\'t remove it the channel will be created again when someone visits the page containing [xen-chat] shortcode.');
    }
	
	public function backupChannelAction() {
		$channel = stripslashes($_GET['channel']);
		$channelStripped = preg_replace("/[^[:alnum:][:space:]]/ui", '', $channel);
		$filename = "XenChatChannelBackup-{$channelStripped}.csv";
	...
	public function backupPrivateChannelAction() {
		$channel = stripslashes($_GET['channel']);
		$channelStripped = preg_replace("/[^[:alnum:][:space:]]/ui", '', $channel);
		$filename = "XenChatChannelBackupPrivate-{$channelStripped}.csv";
	...
	public function channelsCallback() {
		$url = admin_url("options-general.php?page=".XenChatSettings::MENU_SLUG);
		...
		foreach ($summary as $key => $channel) {
			$isDirect = $channel->channel === XenChatChannelsService::PRIVATE_MESSAGES_CHANNEL;
			...
			$setPasswordURL = admin_url("options-general.php?page=".XenChatSettings::MENU_SLUG."&wc_action=setChannelPassword&tab=channels&channel=".urlencode($channel->channel));
			$deletePasswordURL = admin_url("options-general.php?page=".XenChatSettings::MENU_SLUG."&wc_action=deleteChannelPassword&tab=channels&channel=".urlencode($channel->channel));
	...
	public function adminActionsCallback() {
		$url = admin_url("options-general.php?page=".XenChatSettings::MENU_SLUG."&wc_action=clearAllChannels");
