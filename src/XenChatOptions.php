...
/**
 * XenChat class for accessing plugin options.
 *
 * @author Kainex <contact@kainex.pl>
 */
class XenChatOptions {
	const OPTIONS_NAME = 'xen_chat_options_name';
	const LAST_NAME_ID_OPTION = 'xen_chat_last_name_id';
	...
	*/
	...
	private function __construct() {
		$this->options = get_option(XenChatOptions::OPTIONS_NAME);
		if ($this->options === false) {
			$this->options = array();
		}
		add_action('update_option_'.XenChatOptions::OPTIONS_NAME, array($this, 'onOptionsUpdated'), 10, 3);
	}
	
	public static function getInstance() {
		if (self::$instance === null) {
			$instance = new XenChatOptions();
	...
	*/
	public function saveOptions() {
		update_option(XenChatOptions::OPTIONS_NAME, $this->options);
	}
	...
	public function onOptionsUpdated($oldValue, $value, $option) {
		delete_transient('xen_chat_wp_users_cache');
	}
	...
	*/
	public function dropAllOptions() {
		delete_option(XenChatOptions::OPTIONS_NAME);
		delete_option(XenChatOptions::LAST_NAME_ID_OPTION);
	}
