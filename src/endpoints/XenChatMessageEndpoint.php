...
/**
 * Xen Chat message actions endpoint class.
 *
 * @author Kainex <contact@kainex.pl>
 */
class XenChatMessageEndpoint extends XenChatEndpoint {
	...
	public function messageEndpoint() {
		...
		try {
			...
			/** @var XenChatCommandsResolver $xenChatCommandsResolver */
			$xenChatCommandsResolver = XenChatContainer::get('commands/XenChatCommandsResolver');

			// resolve a command if it is recognized:
			$isCommandResolved = $xenChatCommandsResolver->resolve(
	...
	public function getMessageEndpoint() {
		...
		try {
			...
			$channelEncryptedIds = array(
				$channel->getName() => XenChatCrypt::encryptToString('c|'.$channel->getId())
			);
