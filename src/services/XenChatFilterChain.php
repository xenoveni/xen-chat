...
/**
 * Xen Chat text filtering service.
 *
 * @author Kainex <contact@kainex.pl>
 */
class XenChatFilterChain {
	/**
	* @var XenChatOptions
	*/
	private $options;
	...
	public function filter($text) {
        $filtersChain = XenChatContainer::get('dao/XenChatFiltersDAO')->getAll();
		...
		foreach ($filtersChain as $filter) {
			...
			if ($type == 'text') {
				...
				$matches = array();
				$replaceSource = '/'.XenChatFiltersDAO::URL_REGEXP.'/i';
