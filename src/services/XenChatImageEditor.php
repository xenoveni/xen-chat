...
/**
 * Xen Chat images editor class.
 *
 * @author Kainex <contact@kainex.pl>
 */
class XenChatImageEditor {
	/**
	 * GD Resource.
	 *
	 * @var resource
	...
	*/
	public function load($imagePath) {
		if (!file_exists($imagePath)) {
			throw new Exception('XenChatImageEditor: File does not exist');
		}
		
		$imageData = getimagesize($imagePath);
		if ($imageData === false) {
			throw new Exception('XenChatImageEditor: The file is not an image');
		}
		...
		switch ($imageData[2]) {
			...
			case IMAGETYPE_WBMP:
				...
				break;
			default:
				throw new Exception('XenChatImageEditor: Image format is not supported');
		}
	...
	*/
	public function resize($maxWidth, $maxHeight, $constraint = true) {
		if ($this->image === null) {
			throw new Exception('XenChatImageEditor: Image was not loaded');
		}
